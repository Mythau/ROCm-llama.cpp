...
| `--spec-ngram-mod-pool-update MODE` | when to update the shared ngram-mod pool: eligible = proposal-eligible requests only, loaded = all generating requests while ngram-mod is loaded (default: loaded)<br/>(env: LLAMA_ARG_SPEC_NGRAM_MOD_POOL_UPDATE) |
